...
class CalculadoraAPI:
    ...
    def _calcular_parcelamento(self, params, saldo_base):
        ...
        if tipo_calculo == "por_valor":
            ...
            if valor_parcela_desejada <= 0:
                ...
                for _ in range(self.MAX_ITERATION_LOOPS): 
                    ...
                    current_datas_parcelas = self._gerar_datas_parcelas(data_primeiro_pag, num_parcelas, frequencia)
                    ...
                    current_dias_financiamento = (current_data_media - data_primeiro_pag).days
                    current_juros_parcelamento = 0
                    if current_dias_financiamento > 0:
                        current_juros_parcelamento = saldo_base * taxa_dia_decimal * current_dias_financiamento
                    ...
                    num_parcelas = math.ceil(current_valor_total_parcelado / valor_parcela_desejada)
                    num_parcelas = max(1, num_parcelas)
                    num_parcelas = min(num_parcelas, self.MAX_PARCELAS_ITER)
        ...
        actual_final_data_media_idx = min(data_media_idx_int_final, num_parcelas - 1)
    ...
    def gerar_texto_negociacao(self):
        ...
        aplica_multa = any(nf.get('multa', 0) > 0 for nf in details.get("nfs_calculadas", []))
        
        if details.get("parcelas_info"):
            texto_final.append("Pagamento parcelado:")
            for nf in details.get("nfs_calculadas", []):
                valor_nf_atualizado = nf.get('valor_atualizado', nf['valor_original'])
                valor_nf_atualizado = locale.format_string("%.2f", valor_nf_atualizado, grouping=True)
                texto_final.append(f"NF: {nf['num_nf']} - Valor Atualizado: R$ {valor_nf_atualizado}")
            texto_final.append("")
            texto_final.append(f"Saldo para pagamento a vista: R$ {locale.format_string("%.2f", details['saldo_base'], grouping=True)}")
            #texto_final.append(f"Juros do Parcelamento (Data Média): R$ {details.get('juros_parcelamento', 0):.2f}")
            texto_final.append(f"Valor total a parcelar: R$ {locale.format_string("%.2f", details.get('valor_total_parcelado', 0), grouping=True)}")
            texto_final.append("")
            ...
            if len(details.get("nfs_calculadas", [])) == 1:
                ...
                texto_final.append(f"Juros Mora: R$ {nf.get('juros_mora', 0.0):.2f}")
                ...
                if float(details.get('desconto_juros', 0)) > 0:
                    texto_final.append(f"Desconto Juros Mora: R$ {details['desconto_juros']:.2f}")
                texto_final.append(f"Valor total: R$ {details['saldo_base']:.2f}")
            else:
                ...
                for nf in details.get("nfs_calculadas", []):
                    ...
                    texto_final.append(f"NF: {nf['num_nf']} - Valor Atualizado: R$ {valor_nf_atualizado}")
                texto_final.append("")
                texto_final.append(f"Total original: R$ {locale.format_string("%.2f", details['total_principal'], grouping=True)}")
                texto_final.append(f"Total Juros Mora: R$ {locale.format_string("%.2f",details['total_juros'], grouping=True)}")
               ...
               # if float(details.get('desconto_juros', 0)) > 0:
                ...
                newvalor = locale.format_string("%.2f", newvalor, grouping=True)
                texto_final.append(f"Valor total para pagamento a vista: R$ {newvalor}")
